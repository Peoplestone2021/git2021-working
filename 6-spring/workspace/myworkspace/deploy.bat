...
scp -i "c:\keyfile\myworkspace.pem" -r ./build/libs/*.jar ubuntu@ec2-52-78-190-195.ap-northeast-2.compute.amazonaws.com:/home/ubuntu/app/myworkspace
@rem ==		2. jar 파일을 실행하는 run.sh 스크립트 파일을 서버에 전송 
scp -i "c:\keyfile\myworkspace.pem" -r ./run.sh ubuntu@ec2-52-78-190-195.ap-northeast-2.compute.amazonaws.com:/home/ubuntu/app/myworkspace
@rem ==		3. run.sh 스크립트 파일을 실행가능하도록 권한 부여(777 -> rwx rwx rwx)
ssh -i "c:\keyfile\myworkspace.pem" ubuntu@ec2-52-78-190-195.ap-northeast-2.compute.amazonaws.com "sudo chmod 777 /home/ubuntu/app/myworkspace/run.sh"
@rem ==		4. run.sh 스크립트 파일을 실행가능하도록 권한 부여(777 -> rwx rwx rwx)
ssh -i "c:\keyfile\myworkspace.pem" ubuntu@ec2-52-78-190-195.ap-northeast-2.compute.amazonaws.com "cd /home/ubuntu/app/myworkspace; ./run.sh myworkspace"
