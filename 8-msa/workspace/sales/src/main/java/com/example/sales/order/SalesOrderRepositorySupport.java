...
//		where so.date between '1997-01-01' and '1997-01-31';		
		...
		return query.select(salesOrderDetail.quantity.longValue().multiply(salesOrderDetail.unitPrice).sum())
				.from(SalesOrderDetail).join(salesOrder).on(salesOrderDetail.salesOrderId.eq(salesOrder.id))
				.where(salesOrder.date.between(startDate, endDate)).fetchOne();
	}
...
//		order by 1 asc;	
		...
		// .as("별칭")
		return query
				.select(Projections.fields(AmountByDate.class, salesOrder.date,
						salesOrderDetail.quantity.longValue().multiply(salesOrderDetail.unitPrice).sum().as("amount")))
				.from(salesOrderDetail).join(salesOrder).on(salesOrderDetail.salesOrderId.eq(salesOrder.id))
				.where(salesOrder.date.between(startDate, endDate)).groupBy(salesOrder.date)
				.orderBy(salesOrder.date.asc()).fetch();
	}
...
//		order by 1 asc;
		...
		return query
				.select(Projections.fields(AmountByCategory.class, product.category,
				...
				.where(salesOrder.date.between(startDate, endDate)).groupBy(product.category)
				.orderBy(product.category.asc()).fetch();
	}
}
