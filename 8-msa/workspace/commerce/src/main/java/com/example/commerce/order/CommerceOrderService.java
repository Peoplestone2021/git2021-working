...
public class CommerceOrderService {
	...
	@Autowired
	public CommerceOrderService(CommerceOrderRepository orderRepo, CommerceOrderDetailRepository detailRepo,
	...
	public CommerceOrder saveOrder(CommerceOrderRequest reqOrder) {
		...
		// 요청객체 -> 엔티티 객체로 변환
		CommerceOrder toSaveOrder = CommerceOrder.builder().name(reqOrder.getName()).address(reqOrder.getAddress())
				.date(reqOrder.getDate()).totalAmount(total).status("00") // 주문 금액 합계
		...
		for (CommerceOrderRequest.CommerceOrderDetail reqDetail : reqOrder.getDetails()) {
			CommerceOrderDetail detail = CommerceOrderDetail.builder().commerceOrderId(savedOrder.getId()) // 상위 레코드의
																											// id값
					.seq(reqOrder.getDetails().indexOf(reqDetail) + 1) // 주문 제품 순번
					.product(Product.builder().id(reqDetail.getProductId()).build()) // 주문 제품
					.productName(reqDetail.getProductName()).unitPrice(reqDetail.getUnitPrice())
					.quantity(reqDetail.getQuantity()).build();
